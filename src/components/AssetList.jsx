    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        {Object.keys(assetTypes).map((k) => (
          <AddBtn key={k} onClick={() => add(k)}>Add {assetTypes[k]?.name || k}</AddBtn>
        ))}
      </div>
      <div className="space-y-2">
        {assets.map((a) => (
          <div key={a.id} className="grid md:grid-cols-12 gap-2 items-end bg-zinc-900/60 border border-zinc-800 rounded-xl p-3">
            ...
            <TextInput className="md:col-span-3" label="Name" value={a.name} onChange={(v) => update(a.id, { name: v })} />
            <TextInput className="md:col-span-4" label="Description" value={a.description || ""} onChange={(v) => update(a.id, { description: v })} />
            <TextInput className="md:col-span-2" label="Value" type="number" value={String(a.value)} onChange={(v) => update(a.id, { value: Number(v || 0) })} />
