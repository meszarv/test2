...
export default function AddLiabilityModal({ open, onClose, liabilityTypes, onAdd }) {
  const [name, setName] = useState("");
  const [type, setType] = useState(Object.keys(liabilityTypes)[0] || "");
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4">
      <form onSubmit={submit} className="bg-zinc-900 rounded-xl p-4 w-full max-w-sm space-y-3">
        <h2 className="text-lg font-medium">Add liability</h2>
        ...
        <label className="block text-sm">
          ...
          >
            {Object.keys(liabilityTypes).map((k) => (
              <option key={k} value={k}>{liabilityTypes[k]?.name || k}</option>
            ))}
          </select>
        </label>
        <label className="block text-sm">
