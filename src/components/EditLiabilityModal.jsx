...
export default function EditLiabilityModal({ open, liability, onClose, liabilityTypes, onSave, onDelete }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4">
      <form onSubmit={submit} className="bg-zinc-900 rounded-xl p-4 w-full max-w-sm space-y-3">
        <h2 className="text-lg font-medium">Edit liability</h2>
        ...
        <label className="block text-sm">
          ...
          >
            {Object.keys(liabilityTypes).map((k) => (
              <option key={k} value={k}>
                {liabilityTypes[k]?.name || k}
              </option>
            ))}
          </select>
        </label>
        ...
        <div className="flex justify-between gap-2 pt-2">
          ...
          <div className="flex gap-2">
            ...
            <button type="submit" title="Save" className="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500">
              💾
            </button>
          </div>
        </div>
