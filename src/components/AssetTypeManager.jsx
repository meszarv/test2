...
export default function AssetTypeManager({ assetTypes, setAssetTypes, assets, dimensions }) {
  ...
  function addType() {
    ...
    if (!name) return;
    setAssetTypes({ ...assetTypes, [mkId()]: { name, scopeRule: { mode: "user", value: "" }, dimensionRules: {} } });
  }
      ...
      {Object.entries(assetTypes).map(([typeKey, definition]) => (
        <div key={typeKey} className="border border-zinc-800 bg-zinc-900/60 rounded-xl p-4 space-y-3">
          ...
          <div className="space-y-2">
            <div className="text-xs uppercase tracking-wide text-zinc-500">Portfolio scope rule</div>
            <div className="grid grid-cols-[1fr_10rem_1fr] gap-2 items-center">
              <span className="text-sm text-zinc-300">Portfolio view</span>
              <select
                value={definition.scopeRule?.mode || "user"}
                onChange={(event) => updateScopeRule(typeKey, { mode: event.target.value })}
              ...
              <select
                value={definition.scopeRule?.value || ""}
                onChange={(event) => updateScopeRule(typeKey, { value: event.target.value })}
                disabled={!definition.scopeRule || definition.scopeRule.mode === "user"}
